...
const EditEvent = ({ EventId }: any) => {
  const { data, isSuccess: GetEventSuccess } =
    useGetAdminEventQuery<any>(EventId);
  const [updateEvent, { isLoading: updateLoading, isSuccess, error }] =
    useEditEventMutation<any>();
  ...
  const FormatDateFromObjectToString = (dateObj: any) => {
  ...
  const FormatDateFromStringToObject = (dateString: any) => {
  ...
  useEffect(() => {
    if (GetEventSuccess && data) {
      setImage(data?.eventProductImg);
      setStart(FormatDateFromStringToObject(data?.startTime));
      setEnd(FormatDateFromStringToObject(data?.endTime));
  ...
  }, [error, isSuccess, GetEventSuccess]);
  ...
  return (
    <div className="flex flex-col w-[80%] mx-auto mt-8 gap-6">
      <Heading title="Edit Event - limited.eg" />
